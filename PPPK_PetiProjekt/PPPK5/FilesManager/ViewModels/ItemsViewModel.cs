...
namespace FilesManager.ViewModels
{
    class ItemsViewModel
    {
        ...
        private void Refresh()
        {
            ...
            {
                if (item.Name.Contains(ForwardSlash)) //ako ime blob-a sadrzi /, odnosno to je direktorij
                {
                ...
                if (string.IsNullOrEmpty(Directory) && !item.Name.Contains(ForwardSlash)) //ako je u root-u
                {
                    Items.Add(item);
                }
                else if (!string.IsNullOrEmpty(Directory) && item.Name.Contains($"{Directory}{ForwardSlash}")) //ako postoji direktorij i ako ime blob-a sadrzi ime direktorija/
                {
                    Items.Add(item);
                }
            });
        ...
        {
            string filename = path.Substring(path.LastIndexOf(Path.DirectorySeparatorChar) + 1); // /../../image.jpg -> filename
            string directory = filename.Substring(filename.LastIndexOf(Dot) + 1); // image.jpg -> jpg -> directory
